...
const HOST_NAME = 'https://www.w3cschool.cn'
const $ = new Env('编程狮签到')
...
const checkin = (cookie) => {
    return new Promise(resolve => {
        const option = {
            url: `${HOST_NAME}/my/checkin`,
            headers: {
                'cookie': cookie,
                'Referer': `${HOST_NAME}/my`,
...
!(async () => {
    console.log(process.env.BCS_COOKIE)
    if (process.env.BCS_COOKIE) {
        const cookies = process.env.BCS_COOKIE.split('\n')
